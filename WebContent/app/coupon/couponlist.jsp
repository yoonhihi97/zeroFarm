				...
				<div class="col-lg-12">
					<nav class="navbar navbar-expand-lg navbar-light">
						<a class="navbar-brand"
							href="${pageContext.request.contextPath}/user/Index.us"> <img
							src="${pageContext.request.contextPath}/aranoz-master/aranoz-master/img/logo.png"
							alt="logo">
						</a>
						...
						<div class="collapse navbar-collapse main-menu-item"
							...
							<ul class="navbar-nav">
								<li class="nav-item dropdown"><a
									...
									aria-haspopup="true" aria-expanded="false"> 기업소개 </a>
									<div class="dropdown-menu" aria-labelledby="navbarDropdown_1">
										<a class="dropdown-item"
											href="${pageContext.request.contextPath}/company_info/contact.bo">오시는길</a>
										<a class="dropdown-item"
											href="${pageContext.request.contextPath}/company_info/bestfunding.bo">BEST
											펀딩사례</a>
									</div></li>
								<li class="nav-item dropdown"><a
									class="nav-link dropdown-toggle" href="blog.html"
									id="navbarDropdown_1" role="button" data-toggle="dropdown"
									aria-haspopup="true" aria-expanded="false"> 펀딩하기 </a>
									<div class="dropdown-menu" aria-labelledby="navbarDropdown_1">
										<a class="dropdown-item"
											href="${pageContext.request.contextPath}/funding/fundInsert.fd">펀딩
											등록</a> <a class="dropdown-item"
											href="${pageContext.request.contextPath}/funding/fundList.fd">진행중인
											펀딩</a> <a class="dropdown-item"
											href="${pageContext.request.contextPath}/funding/funding-single.fd">종료된
											펀딩</a>

									</div></li>
								<li class="nav-item dropdown"><a
									class="nav-link dropdown-toggle"
									href="${pageContext.request.contextPath}/board/NoticeBoardList.bo"
									...
									<div class="dropdown-menu" aria-labelledby="navbarDropdown_2">
										<a class="dropdown-item"
											...
											href="${pageContext.request.contextPath}/board/EventBoardList.bo">이벤트</a>
										<a class="dropdown-item"
											href="${pageContext.request.contextPath}/FAQ/faq.bo">FAQ</a>
										<!-- <a class="dropdown-item" href="cart.html">펀딩준비방법</a> -->
									</div></li>
								<li class="nav-item"><a class="nav-link"
									href="${pageContext.request.contextPath}/campaign/CampaignList.ca?page=1">모금하기</a>
								</li>
								<c:choose>
									<%-- 로그인 안 했을 때 --%>
									<c:when test="${sessionScope.user eq null }">
										<li class="nav-item only_mobile"><a class="nav-link"
											href="${pageContext.request.contextPath}/user/UserJoin.us"
											class="btn_4">회원가입</a></li>
										<li class="nav-item only_mobile"><a class="nav-link"
											href="${pageContext.request.contextPath}/user/UserLoginGo.us"
											class="btn_4">로그인</a></li>
									</c:when>
									<c:otherwise>
										<%-- 로그인 했을 때 --%>
										<li class="nav-item only_mobile"><a class="nav-link"
											href="#"
											onclick="location.href='${pageContext.request.contextPath}/user/Index.us'">메인화면</a>
										</li>
										<%-- 판매자 등록 안되있을 때 --%>
										<c:if test="${sessionScope.seller eq null }">
											<li class="nav-item"><a href="#" class="nav-link"
												onclick="location.href='${pageContext.request.contextPath}/seller/SellerJoin.sl'">판매자
													등록</a></li>
										</c:if>
										<%-- 판매자 등록 되있을 때 --%>
										<li class="nav-item only_mobile"><a class="nav-link"
											href="#"
											onclick="location.href='${pageContext.request.contextPath}/user/UserLogout.us'">로그아웃</a>
										</li>
									</c:otherwise>
								</c:choose>
							</ul>
							...
							<div class="login_part mypage_part" style="float: right;">
								...
								<c:choose>
									...
									<c:otherwise>
										<%-- 로그인이 된 상태라면 --%>
										<h6>
											<strong>${sessionScope.user.getName()}</strong>님 어서오세요
										</h6>
										<a href="#" class="btn_5"
											onclick="location.href='${pageContext.request.contextPath}/user/Index.us'">메인화면</a>
										<a href="#" class="btn_5"
				...
				<dl class="myinfo coupon_info">
					...
					<dd>
						<section class="contact-section">
							...
							<!-- 본문 -->
							<div class="bodytext_area box_inner coupon_wrap">
								<!-- summary : 리더기에서 읽도록 넣은것 -->
								<table class="bbsListTbl"
									summary="제목, 작성자, 날짜, 타입, 조회수 등을 제공하는 표">
									<thead>
										<tr>
									...
									<tbody>
										...
										<c:choose>
											<c:when
												test="${couponlist != null and fn:length(couponlist)>0 }">

												<!-- 보여줄 게시글 목록이 있음 -->
												<!-- for(BoardDTO board : boardList) -->

												<c:forEach items="${couponlist}" var="coupon">
													<c:if test="${sessionScope.user.getEmail()== coupon.email}">
														<tr>
															<td>${coupon.couponcode}</td>
															<td>${coupon.couponname}</td>
															<td>${coupon.couponcreatedate}</td>
															<td>${coupon.couponexpdate}</td>
															<td>${coupon.coupondiscount}</td>
															<td>${sessionScope.user.getEmail()}</td>
															<td><a
																href="${pageContext.request.contextPath }/coupon/CouponUse.co?couponcode=${coupon.couponcode}&email=${coupon.email}">사용</a></td>
													</c:if>
												</c:forEach>
								...
								<div class="pagenation">
									<tr align="center" valign="middle">
										<td><c:if test="${page>1}">
												<a class="arr prev"
													href="${pageContext.request.contextPath}/coupon/CouponList.co?page=${page-1}&email=${sessionScope.user.getEmail()}"></a>
											</c:if> <c:forEach begin="${startPage}" end="${endPage}" step="1" var="i">
												<c:choose>
													<c:when test="${page == i}">
														<strong class="page_strong_num">${i}</strong>
													</c:when>
													<c:otherwise>
														<a class="page_num"
															href="${pageContext.request.contextPath}/coupon/CouponList.co?page=${i}&email=${sessionScope.user.getEmail()}">${i}</a>
													</c:otherwise>
												</c:choose>
