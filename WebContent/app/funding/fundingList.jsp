                        ...
                        <div class="col-lg-12">
                            <div class="product_top_bar d-flex justify-content-between align-items-center">
                                <div class="single_product_menu">
                                    <p><span>${requestScope.totalCnt} </span>개의 펀딩</p>
                                </div>
                    ...
                    <div class="row align-items-center latest_product_inner">
                        ...
                        <c:choose>
                    		<c:when test="${fundingList != null and fn:length(fundingList)>0 }">
								<c:forEach var = "i" begin = "0" end = "${fn:length(fundingList)-1}">
								<c:set value="${fundingList[i].goalprice}" var="goalprice"/>
								<c:set value="${fundingList[i].nowprice}" var="nowprice"/>
								<fmt:parseNumber var="goalprice" type="number" value="${goalprice}" />
								<fmt:parseNumber var="nowprice" type="number" value="${nowprice}" />
								<fmt:parseNumber var="result" type="number" value="${nowprice/goalprice*100}" />
								<jsp:useBean id="today" class="java.util.Date" />
								<fmt:formatDate var="now" value="${today}" pattern="yyyyMMdd" />
								<fmt:parseDate var="bdate" value="${fundingList[i].enddate}" pattern="yyyy-MM-dd" />
								<fmt:formatDate var="bdate2" value="${bdate}" pattern="yyyyMMdd" />
								<fmt:parseNumber var="dateResult" type="number" value="${bdate2-now}" />
									<div class="col-lg-4 col-sm-6">
                            				<div class="single_product_item">
                                			<div style="position:relative; width:100%; padding-bottom: 80%; overflow:hidden;" >
                                				<img src="<c:url value='/app/file/${fundImgList[i].img1}' />" style="max-width:100%; position:absolute" id="fundSumb">
                                			</div>
                                			<div class="single_product_text">
                                    			<h4>${fundingList[i].fundtitle}</h4>
                                    			<div class="bar-container">
                                    				<c:choose>
                                    					<c:when test="${result>100}">
															<div id="Content_bar" class="bar" style="width:100%"></div>
														</c:when>
														<c:otherwise>
															<div id="Content_bar" class="bar" style="width:${result}%"></div>
														</c:otherwise>
													</c:choose>
												</div>
                                    			<p><span id="per">${result}%</span> <span id="num">${nowprice}</span> 원 <span id="dday">${bdate2-now}<span id="kor">일 남음</span></span></p>
                                    			<a href="${pageContext.request.contextPath}/funding/funding-single.fd?fundnum=${fundingList[i].fundnum}" class="add_cart">더 보기</a>
                            	   	 		</div>
                            				</div>
                        			</div>
								</c:forEach>
                            ...
                            <div class="pageination">
                                <nav aria-label="Page navigation example">
                                	<ul class="pagination justify-content-center">
										<li class="page-item">
											<c:if test="${page>1}">
    	                                        <a class="page-link" href="${pageContext.request.contextPath}/funding/fundList.fd?page=${page-1}" aria-label="Previous">
										...
										<c:forEach begin="${startPage}" end="${endPage}" step="1" var="i">
											<c:choose>
												...
												<c:otherwise>
													<a href="${pageContext.request.contextPath}/funding/fundList.fd?page=${i}">${i}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach>
										<c:if test="${page<totalPage}">
											<li class="page-item">
                                        	    <a class="page-link" href="${pageContext.request.contextPath}/funding/fundList.fd?page=${page+1}" aria-label="Next">
